...
namespace fve {
	...
	FveDevice::FveDevice(FveWindow& window) : window{ window } {
		createInstance();
		setupDebugMessenger();
		createSurface();
		pickPhysicalDevice();
	...
	void FveDevice::pickPhysicalDevice() {
	...
	void FveDevice::createCommandPool() {
	...
	void FveDevice::createSurface() { window.createWindowSurface(instance_, &surface_); }

	bool FveDevice::isDeviceSuitable(VkPhysicalDevice device) {
	...
	void FveDevice::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
		createInfo = {};
	...
	void FveDevice::setupDebugMessenger() {
	...
	bool FveDevice::checkValidationLayerSupport() {
		uint32_t layerCount;
	...
	std::vector<const char*> FveDevice::getRequiredExtensions() {
	...
	void FveDevice::hasGflwRequiredInstanceExtensions() {
	...
	bool FveDevice::checkDeviceExtensionSupport(VkPhysicalDevice device) {
	...
	QueueFamilyIndices FveDevice::findQueueFamilies(VkPhysicalDevice device) {
	...
	SwapChainSupportDetails FveDevice::querySwapChainSupport(VkPhysicalDevice device) {
	...
	VkFormat FveDevice::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
	...
	uint32_t FveDevice::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
	...
	void FveDevice::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
	...
	void FveDevice::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
	...
	void FveDevice::copyBufferToImage(
	...
	void FveDevice::createImageWithInfo(
		...
		VK_CHECK(vmaCreateImage(fveAllocator, &imageInfo, &allocCreateInfo, &image, &allocation, &allocInfo));
